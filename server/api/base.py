...
class RestfulHandler(RequestHandler, AlchemyMixin):
    ...
    def write_error(self, status_code: int, **kwargs: Any) -> None:
        ...
        if exc:
            err = exc[1]
            msg = getattr(err, 'log_message', None) or getattr(err, 'reason', None) or str(err)
            if msg:
                reason = msg
        self.set_status(status_code, reason=reason if reason and reason != 'Unknown' else None)
