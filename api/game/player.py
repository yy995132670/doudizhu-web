...
class Player(object):
    ...
    # 断线后保留座位的时间（秒）：在此期间刷新浏览器可重连回房间，超时才真正离开
    ...
    def __init__(self, uid: int, name: str, sex: int = 1, avatar: str = '', **kwargs):
        ...
        self.sex = sex
        self.avatar = avatar
        self.point = int(kwargs.get('point') or 1000)
    ...
    async def on_message(self, code: int, packet: Dict[str, Any]):
        ...
        if code == Pt.REQ_JOIN_ROOM and self.room:
            room_id = packet.get('room', -1)
            if room_id in (-1, self.room.room_id):
                self.room.on_rejoin(self)
                self.room.sync_room()
                return
            if self.state in (State.INIT, State.WAITING, State.GAME_OVER):
                self.leave_room()
    ...
    def on_disconnect(self):
        ...
        if not self.room:
            return
        # 断线宽限：保留座位与手牌，轮到该玩家时自动出牌，允许刷新浏览器重连。
        self._disconnected = True
        logger.info('PLAYER[%s] DISCONNECTED, grace %ds', self.uid, self.DISCONNECT_GRACE_SECONDS)
        if self._leave_handle is None:
            self._leave_handle = IOLoop.current().call_later(
                self.DISCONNECT_GRACE_SECONDS, self._expire_after_disconnect)

    def clear_disconnect(self):
        if self._disconnected or self._leave_handle is not None:
            logger.info('PLAYER[%s] RECONNECTED, cancel grace', self.uid)
        self._disconnected = False
        if self._leave_handle is not None:
            IOLoop.current().remove_timeout(self._leave_handle)
            self._leave_handle = None

    def _expire_after_disconnect(self):
        self._leave_handle = None
        if self._disconnected:
            logger.info('PLAYER[%s] GRACE EXPIRED -> leave room', self.uid)
            self.leave_room()
    ...
    def on_timeout(self):
        if not (self.is_left() or self.is_disconnected()):
            return
    ...
    def handle_leave(self, code: int, packet: Dict[str, Any]):
        ...
        if code == Pt.REQ_JOIN_ROOM:
            ...
            if room and room.room_id == room_id and room.on_rejoin(self):
                room.sync_room()
    ...
    def handle_game_over(self, code: int, packet: Dict[str, Any]):
        if code == Pt.REQ_READY and self.room:
            self.room.restart()
            self.ready = packet.get('ready')
            if self.room.is_ready():
                self.change_state(State.CALL_SCORE)
    ...
    @property
    def timeout(self):
        return 5 if (self.is_left() or self.is_disconnected()) else 20
