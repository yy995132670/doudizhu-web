...
class Room(object):
    ...
    def sync_data(self):
        # 底牌：仅在进入出牌阶段后公开（叫地主阶段仍保密）
        from .player import State
        bottom = list(self.pokers) if int(self.room_state) >= int(State.PLAYING) else []
        return {
            'id': self.room_id,
    ...
    def on_rejoin(self, target: Player) -> bool:
        from .player import State
        for i, player in enumerate(self.players):
            if player == target:
                ...
                target.room = self
                target.set_left(0)
                if target.state == State.INIT:
                    active_state = next((
                        p.state for p in self.players
                        if p and p != target and not p.is_left() and p.state != State.INIT
                    ), State.WAITING)
                    target.state = active_state if target.hand_pokers else State.WAITING
                return True
        if self.is_full():
            target.write_error('Room[%s] FULL' % self.room_id)
            return False
        return target.join_room(self)
    ...
    def on_leave(self, target: Player, is_restart=False):
        ...
        try:
            free_robot = 0
            was_active = int(self.room_state) >= 2
            for i, player in enumerate(self.players):
                if player == target:
                    self.players[i] = None
            ...
            if was_active and not is_restart and not self.is_empty():
                self.cancel_current_game()
    ...
    def cancel_current_game(self):
        ...
        for player in self.players:
            if player:
                player.restart()
        logging.info('Room[%d] cancelled because a player left', self.room_id)
    ...
    def on_game_over(self, winner: Player):
        ...
        self.timer.stop_timing()
        IOLoop.current().add_callback(self.save_points)
    ...
    async def save_points(self):
        async with async_session() as session:
            async with session.begin():
                for player in self.players:
                    if not player or player.uid >= 10000:
                        continue
                    await session.execute(update(User).where(User.id == player.uid).values(point=player.point))

    async def save_shot_round(self):
        for player in self.players:
            if not player or not player.socket:
                continue
            robot = self.has_robot()
            landlord = self.landlord
            round_data = {
                'left': {p.seat: p.hand_pokers for p in self.players if p},
                'round': self.shot_round,
                'lord': landlord.seat if landlord else -1,
            }
            await player.socket.insert(Record(round=round_data, robot=robot))
            break
